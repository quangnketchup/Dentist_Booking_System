...
//            check validation here: checkId, name, role , pass,...password
            ...
            ServiceDTO service = new ServiceDTO(1, serviceName, servicePrice, image, description, adminID, 1, serviceTypeName);

            if (check) {
                boolean checkUpdate = dao.createService(service);
                url = SUCCESS;
                request.setAttribute("SSMSG", "Add thành công !");
            } else {
                request.setAttribute("SSMSG", "Add thất bại !");
            }
        } catch (Exception e) {
            log("Error at add service Controller");
        } finally {
